...
// NOTE: We intentionally do NOT use onSnapshot() on the "fees" or
// "salary" collections here. A realtime listener on a whole
// collection downloads every document (and re-downloads on every
// single change anywhere in it) just to compute one total — that
// is what was making this page slow as those collections grew.
// Instead we fetch just the sum from the server (see below) once
// on load, and again whenever the user presses "Refresh Totals"
// or adds/removes an Other Income / Other Expense entry.

// ------------------------------------
// Fast Server-Side Totals (aggregate)
//
// NOTE: We deliberately avoid downloading every document in
// "fees" / "salary" just to add up one field -- that gets
// very slow as those collections grow into the hundreds or
// thousands of records. getAggregateFromServer() asks
// Firestore to compute the sum on the server and only sends
// back the single number, which is dramatically faster.
// ------------------------------------

async function loadFeesTotal() {

    try {

        const snap = await getAggregateFromServer(feesRef, {
            total: sum("amount")
        });

        feesTotal = snap.data().total || 0;

    } catch (error) {

        console.error("Fees total aggregate failed:", error);
...
async function loadSalaryTotal() {

    try {

        const snap = await getAggregateFromServer(salaryRef, {
            total: sum("netSalary")
        });

        salaryTotal = snap.data().total || 0;

    } catch (error) {

        console.error("Salary total aggregate failed:", error);
...
window.refreshAccountTotals = function () {
...
function renderAll() {
    ...
    const otherIncomeTotal = sum_(otherIncome, "amount");
    const otherExpenseTotal = sum_(otherExpenses, "amount");
