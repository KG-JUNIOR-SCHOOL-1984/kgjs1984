...
async function loadResults() {
    ...
    // Each result doc is one subject. Group subject-wise docs
    // together by exam (+ session) so we can show one row per
    // exam with a combined total, average GPA, and grade —
    // matching how the admin Result List displays results.
    ...
    snapshot.forEach((docSnap) => {
        ...
        const key = `${r.session || ""}|${r.examName || ""}`;
        ...
        examMap.get(key).subjects.push(r);

    });

    const exams = Array.from(examMap.values());

    table.innerHTML = exams.map((exam) => {
        ...
        const avgGPA = exam.subjects.length
            ? (totalGPA / exam.subjects.length).toFixed(2)
            : "0.00";

        const overallGrade = exam.subjects.length
            ? exam.subjects[exam.subjects.length - 1].grade || ""
            : "";
