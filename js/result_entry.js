...
// The "studentId" <select> field's value is the Firestore
// DOCUMENT id (used to look up the student record for
// autofill) — NOT the school-assigned Student ID (e.g.
// "STU0001"). Results must be saved with the real Student ID
// so the student portal's query (which matches on the real
// Student ID) can find them.
const selectedDocId = document.getElementById("studentId").value;

const selectedStudent = studentList.find(
item => item.docId === selectedDocId
);
...
// The <select id="studentId"> uses Firestore docId as its
// option values, but the saved result stores the real
// (school-assigned) Student ID — so look up the matching
// student record by that real ID to select the right option.
const matchedStudent = studentList.find(
item => item.studentId === result.studentId
);
setVal("studentId", matchedStudent ? matchedStudent.docId : "");
